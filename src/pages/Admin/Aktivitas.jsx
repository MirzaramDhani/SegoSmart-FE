...
const App = () => {
  ...
  useEffect(() => {
    ...
    async function fetchingData() {
      ...
      const filteredRating = riwayatList.data.filter(
        (item) => item.kepuasan !== null
      );

      const rating = filteredRating.reduce(
        (accumulator, item) => accumulator + item.kepuasan,
        0
      );

      const ratingCount = filteredRating.length;
      
      setTotalHarga(total);
      const avg = parseFloat(rating/ratingCount);
      setAverageRating(avg);
      console.log (filteredRating,rating,ratingCount,averageRating)
                ...
                return (
                  <>
                    ...
                    {Array.from({ length: emptyStars }, (_, i) => (
                      <i
                        key={`empty-${i}`}
                        className="far fa-star text-gray-400"
                      ></i>
                    ))}
                  </>
                );
